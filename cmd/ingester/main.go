...
func main() {
	...
	conn, err := clickhouse.Open(&clickhouse.Options{Addr: []string{cfg.CHDSN}})
	if err != nil { log.Fatal(err) }
	defer conn.Close()

	ctx := context.Background()
	conn.Exec(ctx, `CREATE TABLE IF NOT EXISTS netintel_events (
		id UUID, version UInt8, source LowCardinality(String), subject String, observed_at DateTime, meta JSON
	...
	for {
		...
		if err := json.Unmarshal(m.Value, &evt); err != nil { log.Println("bad message:", err); continue }
